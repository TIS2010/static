...
jTis.extend({
	...
	loadrecords:function(app,query,sort,success,fail){
		var load=function(app,query,sort,records,offset,success){
			var body={
				app:app,
				query:query
			};
			...
			kintone.api(kintone.api.url('/k/v1/records',true),'GET',body,function(resp){
				...
				offset+=limit;
				if (resp.records.length==limit) load(app,query,sort,records,offset,success);
				else success(records);
			},function(error){fail($.builderror(error));});
		}
...
jTis.fn.inlinebrowser=function(options){
	var InlineBrowser=function(options){
		...
		this.callback=null;
		this.timer=0;
		this.hiddenSelectors=options.hiddenSelectors;
				...
				else
				{
					$('.inlinebrowser-block',this.dialog.contents).attr('src',url).on('load',function(){
						(function(contents){
							...
							$('body',contents).css({'overflow-y':'auto'});
							$('[class*=app-titlebar]',contents).css({'display':'none'});
							$('[class$=app-toolbar]',contents).css({'display':'table'});
							for (var i=0;i<my.hiddenSelectors.length;i++) $(my.hiddenSelectors[i],contents).css({'display':'none'});
